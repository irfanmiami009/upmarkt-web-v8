...
<html lang="en">
   <head>
      ...
      <link href="{{asset('public/assets/front-end')}}/css/theme.min.css" rel="stylesheet">
   ...
   <body>
      ...
      <div>
          <!-- top header -->
          @include('layouts.front-end.partials.topheader')
          <!-- header -->
          @include('layouts.front-end.partials.header')
          <!-- menu -->
          @include('layouts.front-end.partials.menu')
          <!-- Page Content -->
          @yield('content')
          <!-- footer -->
        ...
        function getVariantPrice() {
            if ($('#add-to-cart-form input[name=quantity]').val() > 0 && checkAddToCartValidity()) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        var discount_percentage = parseInt(data.price)-parseInt(data.discount);
                        $('#add-to-cart-form #chosen_price_div').removeClass('d-none');
                        $('#add-to-cart-form #chosen_price_div #chosen_price').html(data.price);
                        $('#set-tax-amount').html(data.tax);
                        $('#set-discount-amount').html(data.discount);
                        $('#stock_text').html(data.stock_text);
                        $('#set-discount-percentage').html(data.discount_percentage);
